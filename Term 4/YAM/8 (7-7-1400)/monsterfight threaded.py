...
def start():
            ...
            for i in my_soldiers:
                if current_soldier == i.name:
                    ...
                    if monster.is_alive == FALSE:
                        ...
                        th1 = threading.Thread(target=www).start()
                        
    def attack2():
        while monster.is_alive:
            names = []
            for i in combo_soldiers['values']:
                names.append(i)
            current_soldier = choice(names)
            if current_soldier == '':
                info_soldier.set('No soldier selected!!!')
            for i in my_soldiers:
                if current_soldier == i.name:
    ...
    btn_attack = Button(play_ground, text = 'attack', command = lambda: attack())
    ...
    th2 = threading.Thread(target=attack2)
    th2.start()    
    refresh()
